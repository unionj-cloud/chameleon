...
func (ta *Table) AddColumn(name string, columnType string, collation string, extra string, colNull string, defaultVal *string, comment string) {
	...
	if strings.HasPrefix(columnType, "float") ||
		...
		ta.Columns[index].Type = TYPE_BOOL
	...
	if colNull == "YES" {
		ta.Columns[index].Nullable = true
	}

	ta.Columns[index].Default = defaultVal
	ta.Columns[index].Comment = comment
}
...
func (ta *Table) fetchColumns(conn mysql.Executer) error {
	...
	for i := 0; i < r.RowNumber(); i++ {
		...
		colNull, _ := r.GetString(i, 3)
		colDefault, _ := r.GetValue(i, 5)
		extra, _ := r.GetString(i, 6)
		comment, _ := r.GetString(i, 8)
		var defaultVal *string
		if colDefault != nil {
			value := cast.ToString(colDefault)
...
func (ta *Table) fetchColumnsViaSqlDB(conn *sql.DB) error {
	...
	for r.Next() {
		...
		ta.AddColumn(name, colType, collation.String, extra, "", nil, "")
	}
